  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        patch_package_json =
          ...
          pkgs.stdenv.mkDerivation {
            ...
            src = builtins.path {
              ...
              filter =
                filepath: type:
                type == "regular" && baseNameOf filepath == "package.json" && dirOf filepath == "${path}";
            };
            buildPhase = ''
              ${patch}
            '';
        ...
        '';
        shebang_patches = ''
          find ./app -type f -exec sed -i 's/\/bin\/bash/${
            lib.strings.escape [ "/" ] "${pkgs.bash}/bin/bash"
          }/g' {} \;
          find ./app -type f -exec sed -i 's/\/usr\/bin\/env python3/${
            lib.strings.escape [ "/" ] "${pkgs.python3}/bin/python"
          }/g' {} \;
        '';
        misc_patches = ''
      ...
      rec {
        ...
        packages = rec {
          zotero-reader = pkgs.stdenv.mkDerivation rec {
            ...
            nativeBuildInputs = shared_build_inputs ++ [
              reader_yarn_deps
              pdfjs_yarn_deps
            ];
